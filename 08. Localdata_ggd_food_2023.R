#실습에 필요한 packages를 라이브러리에  + 작업 한 줄 한줄 씩 다 해야함, 전체 실행시 노트북 다운
library(dplyr)
library(ggplot2)
...
#.파생변수 만들기
#1.status변수 + 이 함수 건너뛰는거 추천, 실행시 R 터짐
...
table(is.na(foodshop$open_date))#결측치
foodshop<-foodshop%>%
  filter(open_date!='')%>%
  select(name,type,status,open_date,close_date,address)
...
#5.address변수
foodshop$district<-substr(foodshop$address,5,7)#시,군 정보를 분리하여 변수 생성
table(foodshop$district)#이상치 확인
foodshop$district <- ifelse(foodshop$district%in%c("밀양","영암","106","번지","시 강","시 계","시 관","시 금","시 남","시 노","시 마","시 미","시 서","시 용","시 은","회"),NA,foodshop$district)#이상치제거
table(foodshop$district)#이상치 확인
